...
namespace Holoi.Reality.Typography
{
    ...
    public class RopeControllerRealisticNoSpring : MonoBehaviour
    {
        ...
        [SerializeField] float noiseAmp = 2f;
        [SerializeField] float noiseFrq = 2f;
        [SerializeField] float ropeHeaderMultipier = 4f;
        [SerializeField] float ropeWidth = 0.2f;
        [SerializeField] private float ropeSectionLength = 0.5f;
        [SerializeField] private int ropeSections = 10;
        [HideInInspector] public Vector3 ropeHeaderDirection;
        ...
        private void UpdateRopeSimulation()
        {
            ...
            for (int i = 1; i < allRopeSections.Count; i++)
            {
                ...
                //Add gravity
                currentRopeSection.pos += gravity * t;
                //Add noise
                currentRopeSection.pos +=
                    new Vector3(
                        Perlin.Noise(
                    currentRopeSection.pos.x + Time.time * noiseFrq),
                    Perlin.Noise(
                    currentRopeSection.pos.y + Time.time * noiseFrq),
                    Perlin.Noise(
