...
{
  [Preserve]
  class HoloKitHandsProvider : XRHandSubsystemProvider
  {
    ...
    static HoloKitHandsProvider() => id = "HoloKit Hands Provider";
    ...
    [Preserve]
    void UpdateData(Handedness handedness, HandData handData, NativeArray<XRHandJoint> handJointArray, ref Pose rootPose)
    {
    ...
    private void UpdateJoint(int jointIndex, int HoloKitJointIndex, Handedness handedness, HandData handData, NativeArray<XRHandJoint> handJointArray)
    {
      handJointArray[jointIndex].TryGetPose(out var pose);
      pose.position = handData.joints[HoloKitJointIndex].position;
      pose.rotation = handData.joints[HoloKitJointIndex].rotation;
      ...
      handJointArray[jointIndex] = XRHandProviderUtility.CreateJoint(
          ...
          pose,
          handData.joints[HoloKitJointIndex].radius);
    }

    private void UpdatePalmJoint(int jointIndex, Handedness handedness, HandData handData, NativeArray<XRHandJoint> handJointArray)
    {
      int HoloKitJointIndex = 10; // Middle finger metacarpal
      handJointArray[jointIndex].TryGetPose(out var pose);
      pose.position = handData.joints[HoloKitJointIndex].position;
      pose.rotation = handData.joints[HoloKitJointIndex].rotation;
    ...
    public void UpdateHandJoints(HandData handData)
    {
      if (handData.hand == 1)
      {
